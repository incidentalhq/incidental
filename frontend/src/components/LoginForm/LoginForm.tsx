...
const validationSchema = Yup.object().shape({
  ...
  password: Yup.string().required('A password is required').min(8, 'Your password must be at least 8 characters long')
})
const placeholder = names[getRandomInt(1, names.length - 1)].toLowerCase().replace(' ', '.') + '@gmail.com'

const LoginForm: React.FC<LoginFormProps> = ({ onSubmit }) => {
  return (
    <Formik validationSchema={validationSchema} initialValues={{ emailAddress: '', password: '' }} onSubmit={onSubmit}>
      {({ isSubmitting }) => (
