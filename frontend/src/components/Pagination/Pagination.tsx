...
const Page = styled.a<{ selected: boolean }>`
  padding: 0 1rem;
  color: ${(props) =>
    props.selected ? "var(--color-blue-600)" : "var(--color-gray-500)"};
...
const NextLink: React.FC<NextPreviousLinkProps> = ({
  page,
  totalPages,
  onClick,
}) => {
  ...
  return (
    <Page
      selected={false}
      href="#next"
      onClick={createClickHandler(page + 1, onClick)}
    >
      Next page
    </Page>
  );
...
const PageLinks: React.FC<PageLinkProps> = ({
  currentPage,
  pages,
  onClick,
}) => {
        ...
        nodes.push(
          <li key={page}>
            <Page
              selected={false}
              href={`#page-${page}`}
              onClick={createClickHandler(page, onClick)}
            >
              {page}
            </Page>
          </li>
