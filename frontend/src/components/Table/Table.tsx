...
const TableRow = <RecordType,>({ row, columns, onRowClick }: React.PropsWithChildren<RowProps<RecordType>>) => {
  ...
  return (
    <tr className={rowClass} onClick={handleClick}>
      {columns.map((col) => (
        <td key={col.name} className={getColumnCssClass(col)} style={getStyle(col)}>
...
const Table = <T extends Record<string, unknown>>({
  ...
  const rowKeyFunc = typeof rowKey === 'function' ? rowKey : (row: T) => row[rowKey] as string
  ...
  return (
    <table className={styles.root}>
      ...
      <tbody>
        {data.map((row) => (
          <TableRow<T> key={rowKeyFunc(row)} row={row} columns={columns} onRowClick={onRowClick} />
        ))}
      </tbody>
    </table>
  )
